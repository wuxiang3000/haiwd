$(function(){
	...
	if (bodyId == "ap") {
		...
		$.get("/goform/WifiStatistic",function(str) {
			if(str != " "){
			var datas = str.split("\n");
			if(curmode == '2'){
				if(datas.length == 8){
			...
			for(i = 0; i < datas.length; i++){
				...
				tabTb.insertRow(i);
				tabTb.rows[i].insertCell(0).innerHTML = (rowlist[0]);//.replace(/\s/g,"&nbsp;")
	...
	if (bodyId == "ap_5g") {
		$("#head").html(tbl_head(2));
		var tabTb = document.getElementById("StatisticList");
		$.get("/goform/WifiStatistic5g",function(str) {
			if(str != " "){
			var datas = str.split("\n");
			if(curmode == '2'){
				if(datas.length == 4){
	...
	if (bodyId == "wirelessList") {
		...
		var ssidList = document.getElementById("index");
		ssidList.options[0] = new Option($("<span>").html(data.ssid1).text(),"0");
		ssidList.options[1] = new Option($("<span>").html(data.ssid2).text(),"1");
		ssidList.options[2] = new Option($("<span>").html(data.ssid3).text(),"2");
		ssidList.options[3] = new Option($("<span>").html(data.ssid4).text(),"3");
		ssidList.options[4] = new Option($("<span>").html(data.ssid5).text(),"4");
		ssidList.options[5] = new Option($("<span>").html(data.ssid6).text(),"5");
		ssidList.options[6] = new Option($("<span>").html(data.ssid7).text(),"6");
		if(curmode != 2) {
		ssidList.options[7] = new Option($("<span>").html(data.ssid8).text(),"7");
		} 
...
function initValue(index,radio,flag){
	...
	$.get("/goform/WifiClientList?index=" + ssidIndex + "&wl_radio=" + radio,function(str){
		...
		if (datas.length < 2) {
			...
			for (i = 0; i < datas.length - 1; i++) {
				...
				tbd.rows[i].insertCell(2).innerHTML = dataStr[1] != "0.0.0.0" ? dataStr[1] : _("Unknow");
