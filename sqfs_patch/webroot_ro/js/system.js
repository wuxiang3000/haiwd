...
$(function() {
	...
	switch (op_id) {
		...
		case 'system_upgrade':
			...
			$("#fireware").html( _("Current Firmware Version: %s;  Release Date: %s", [FirwareVerion,FirwareDate] ) );
			break;
		case 'system_reboot':
	...
	switch (op_id) {
		case 'system_restore':
			if(window.confirm( _("Device will restart automatically! The IP address will be reset to 192.168.0.254.") + 
				...
				$.get("/goform/SysToolRestoreSet",function(){});
...
function submitSystemSnmp() {
	...
	if(f.snmpEn[1].checked == true){
		...
		if (!checkData(f.snmpSetcomm.value,_("Write/Read Community")))	return;
		if(f.snmpGetcomm.value != "private" && f.snmpGetcomm.value != "public") {
			alert( _("Read Community must be 'private' or 'public'!"));
			return false;	
		}
		if(f.snmpSetcomm.value != "private" && f.snmpSetcomm.value != "public") {
			alert( _("Write/Read Community must be 'private' or 'public'!"));
			return false;
		}
		if(f.snmpGetcomm.value == f.snmpSetcomm.value) {
			alert(_("Read Community should not be the same as that of Write/Read Community!"));
			return false;	
		}
	}
	f.submit();
...
function submitSystemReboot() {
	...
	{
		var url = "http://" + lanip;
		$.get("/goform/SysToolReboot",function(){
...
function submitSystemHostname() {
	var sy = +f.year.value,
		...
		ss = +f.second.value;
	if (sy < 1970 || sy > 2037) {
		alert(_("The year must be between 1970～2037!"));
		return false;
	}
	if(smo < 1 || smo > 12) {
		alert(_("Please enter a valid month!"));
		return false;
	}
	if(smo == 2) {//2月
		if (sd > 28) {
			if (((sy%4 == 0 && sy%100 != 0) || sy%400 == 0) && sd == 29) {
			} else {
...
function diagnosis() {
	...
	$.get("/goform/CheckTools?ipaddress="+ipaddress+"&selectcmd="+selectcmd,function(){
...
function submitSystemChecktool() {
	...
	if (!/^ping\s\S+$/.test(f.cmdinput.value)) {
		alert(_("Please enter valid value."));
		return false;
	}
	if (/^[\d.]+$/.test(str)) {
		if (str.split(".").length != 4 || +str.split(".")[0] == 0 || str.split(".")[1] == "" ||
				str.split(".")[2] == "" || str.split(".")[3] == "")  {
			alert(_("Please enter a valid IP or domain name!"));
			return false;
		}
	}
	if(/^[0-9.]{1,}$/.test(str)){
